...
namespace Quan_ly_nhan_su
{
    public partial class QuanliNhanvien : Form
    {
        ...
        private void cb_hoten_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            NhanVien nv =( from p in data.NhanViens where p.HoTen == tennv select p).FirstOrDefault();
            cb_chucvu.Text = (from p in data.ChucVus where p.MaCV == nv.MaCV select p.TenCV).FirstOrDefault();
            cb_luong.Text = Convert.ToString((from p in data.Luongs where p.MaLuong == nv.MaLuong select p.LuongCB).FirstOrDefault());
            cb_trinhdo.Text = (from p in data.TrinhDoHVs where p.MaTD == nv.MaTD select p.TenTDHV).FirstOrDefault();
